...
# ======================================================
# Snapshot Backup - v5 (Verified Engine)
# ======================================================
...
#     covered the rsync transfer itself -- the sanity check, hard-link
#     verification, and manifest building were not measured.
#   - Improved messaging for manifest baseline run: when no previous
#     manifest exists (first run with this feature, or after manifest
#     corruption fallback), the output now explains why everything
#     is being hashed and that future runs will be fast. Previously
#     "0 inherited, 36579 hashed" appeared with no explanation and
#     looked like a bug rather than expected first-run behavior.
#
...
# Checks the common udisks2 automount path first (KDE Plasma, GNOME),
# then falls back to other locations for minimal/XFCE/Debian setups.
...
if [[ " $NO_HARDLINK_FS " == *" $FS_TYPE "* ]]; then
    ...
    read -p "Continue anyway? (y/N): " fs_confirm
...
# SOURCES
# -----------------------------
# Reads from ~/.config/snapshot-backup/sources.conf if it exists.
# One path per line, # for comments, ~ expands to $HOME.
# Falls back to the hardcoded default list if no config file exists.
...
# Per-folder excludes via rsync's own merge filter.
# Drop a .backupignore file in any source folder and rsync applies
# its patterns (one per line) scoped to that folder and below.
...
# -----------------------------
# rsync --dry-run reports actual new bytes to be written (after
# hard-link dedup), unlike du which counts every source byte.
...
# -----------------------------
# NUL-delimited with inode first: safe for filenames containing spaces.
# Confirmed by testing: the previous space-delimited version falsely
# reported different-inode files as linked when they shared a
# multi-word filename.
if [ -n "$PREV" ]; then
    echo ""
    ...
    # -----------------------------
    # Internal metadata files are excluded from both sides.
    # Without this, .snapshot_complete would always appear as "deleted"
    # because it hasn't been written to the new snapshot yet when this
    # comparison runs (it's created further down in the script).
    echo ""
...
# -----------------------------
# 20 files out of ~36,000 is signal, not proof. Answers:
# "Did this backup probably succeed?" not "Is every file intact?"
# For the latter, see .snapshot_manifest.sha256 below.
...
# -----------------------------
# Sanity check answers: "Did this backup probably succeed?" (today)
# This manifest answers: "Is this snapshot intact?" (years from now)
#
# To verify any snapshot at any future point:
#   cd /path/to/Backup_<timestamp>
#   sha256sum --check .snapshot_manifest.sha256
#
# Incremental: files whose inode+size+mtime match the previous
# snapshot inherit their checksum for free. Only new/changed files
# are actually read and hashed. First run hashes everything once;
# subsequent runs only hash what changed.
echo ""
...
if [ -n "$PREV" ] && [ -f "$PREV_MANIFEST_PATH" ]; then
    # Verify the previous manifest has at least one well-formed line
    # before trusting it. A corrupt or truncated manifest would
    # otherwise silently propagate bad checksums into all future ones.
...
if [ "$PREV_MANIFEST_OK" -eq 1 ]; then
    # Incremental run: inherit checksums from previous manifest
    # where inode + size + mtime all match.
    (cd "$SNAPSHOT_ROOT/$PREV" && find . -type f \
        ! -name ".snapshot_manifest.sha256" ! -name "$COMPLETE_MARKER" \
        -printf "%i %s %Ts %P\0") > "$MANIFEST_PREV_INODE_FILE"

    # Build NUL-delimited checksum lookup: "checksum path\0"
    # sha256sum format is: 64-char checksum + 2 spaces + path.
    # Checksum is first (no spaces in it), so spaced filenames are safe.
    awk '{
        ck   = substr($0, 1, 64)
        path = substr($0, 67)
        sub(/^\.\//, "", path)
    ...
    # Baseline run: no previous manifest (first run with this feature,
    # or previous manifest failed integrity check). Hashing everything
    # from scratch. Future runs inherit from this manifest and only
    # hash files that actually changed.
    if [ -n "$PREV" ]; then
...
# Name must match exact timestamp format before any rm -rf touches it.
